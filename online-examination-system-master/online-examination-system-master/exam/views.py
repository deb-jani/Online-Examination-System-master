...
#Report Generation 
    ...
    #admin_view_teacher report
def techerpdf(request):
    teachers = TMODEL.Teacher.objects.all().filter(status=True)
    template_path = 'exam/techerpdf.html'
    context = {'teachers':teachers,"messages":teacher}
...
def viewstudentpdf(request):
    students= SMODEL.Student.objects.all()
    template_path = 'exam/viewstudentpdf.html'
    context = {'students':students,"messages":students}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="viewstudentpdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
...
def viewteachersalarypdf(request):
    teachers = TMODEL.Teacher.objects.all().filter(status=True)
    template_path = 'exam/viewteachersalarypdf.html'
    context = {'teachers':teachers,"messages":teachers}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="viewteachersalarypdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
...
def viewstudentmarkspdf(request):
    students= SMODEL.Student.objects.all()
    template_path = 'exam/viewstudentmarkspdf.html'
    context = {'students':students,"messages":students}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="viewstudentmarkspdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
...
def viewquestionpdf(request):
    courses= models.Course.objects.all()
    template_path = 'exam/viewquestionpdf.html'
    context = {'courses':courses,"messages":courses}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="viewquestionpdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
...
def viewpendingteacherpdf(request):
    teachers = TMODEL.Teacher.objects.all().filter(status=False)
    template_path = 'exam/viewpendingteacherpdf.html'
    context = {'teachers':teachers,"messages":teachers}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="viewpendingteacherpdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
...
def viewmarkspdf(request):
    courses = models.Course.objects.all()
    template_path = 'exam/viewmarkspdf.html'
    context = {'courses':courses,"messages":courses}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="viewmarkspdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
...
def viewcoursepdf(request):
    courses = models.Course.objects.all()
    template_path = 'exam/viewcoursepdf.html'
    context = {'courses':courses,"messages":courses}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="viewcoursepdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
...
def checkmarkspdf(request):
    results= models.Result.objects.all()
    template_path = 'exam/checkmarkspdf.html'
    context = {'results':results,"messages":results}
    response = HttpResponse(content_type='application/pdf')
    response['content-Disposition'] = 'attachment; filename="checkmarkspdf.pdf"'
    template = get_template(template_path)
    html = template.render(context)
    #create pdf
    pisa_status = pisa.CreatePDF(html, dest=response)
